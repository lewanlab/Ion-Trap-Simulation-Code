...
%NH_Ca = NeutralHeating(Ca40,HeatRate('He',DarkMass,Initial_T)*1000);
%NH_DarkIon = NeutralHeating(Dark,HeatRate('He',DarkMass,Initial_T)*1000);
%sim.Add (NH_Ca);
%sim.Add (NH_DarkIon);
sim.Add(lasercool);
...
%Print total energy, secular temperature and vrms velocities to a file 
...
if ImgSim == 1
    ...
    fprintf(PfileID,'\n');
    fprintf(PfileID,'%e ', LastDarkx);
    fprintf(PfileID,'\n');
    fprintf(PfileID,'%e ', LastDarky);
    fprintf(PfileID,'\n');
    fprintf(PfileID,'%e ', LastDarkz);
